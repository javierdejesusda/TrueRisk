...
export function ForecastChart({ data, isLoading }: Props) {
  if (isLoading) {
    return (
      <ModelCard
        title="Multi-Horizon Risk Forecast"
        subtitle="Temporal Fusion Transformer probabilistic forecast"
        methodology="Temporal Fusion Transformers produce quantile forecasts (p10/p50/p90) at 6 horizons from +6h to +7d. A Graph Attention Network refines predictions using inter-province spatial context."
        badge={{ label: 'TFT+GNN', variant: 'info' }}
  ...
  if (!data || data.hazards.length === 0) {
    return (
      <ModelCard
        ...
        subtitle="Temporal Fusion Transformer probabilistic forecast"
        methodology="Temporal Fusion Transformers produce quantile forecasts (p10/p50/p90) at 6 horizons from +6h to +7d. A Graph Attention Network refines predictions using inter-province spatial context."
        badge={{ label: 'TFT+GNN', variant: 'neutral' }}
        className="lg:col-span-3 md:col-span-2"
        index={0}
      >
        <div className="flex items-center justify-center h-[200px]">
          <p className="font-[family-name:var(--font-sans)] text-sm text-text-muted">
            No forecast data available. TFT models have not been trained yet.
          </p>
        </div>
      </ModelCard>
    );
  ...
  return (
    <ModelCard
      title="Multi-Horizon Risk Forecast"
      subtitle="Temporal Fusion Transformer probabilistic forecast"
      methodology="Temporal Fusion Transformers produce quantile forecasts (p10/p50/p90) at 6 horizons from +6h to +7d. A Graph Attention Network refines predictions using inter-province spatial context. Shaded bands show uncertainty range."
      badge={{ label: 'TFT+GNN', variant: 'info' }}
      className="lg:col-span-3 md:col-span-2"
      index={0}
    >
